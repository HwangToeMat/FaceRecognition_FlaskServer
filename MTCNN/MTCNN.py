...
def create_mtcnn_net(image, mini_face, device, p_model_path=None, r_model_path=None, o_model_path=None):
    ...
    if p_model_path is not None:
        ...
        pnet.load_state_dict(torch.load(
            p_model_path, map_location=lambda storage, loc: storage), strict=False)
        pnet.eval()
    ...
    if r_model_path is not None:
        ...
        rnet.load_state_dict(torch.load(
            r_model_path, map_location=lambda storage, loc: storage), strict=False)
        rnet.eval()
    ...
    if o_model_path is not None:
        onet = ONet().to(device)
        onet.load_state_dict(torch.load(
            o_model_path, map_location=lambda storage, loc: storage), strict=False)
        onet.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test_image", dest='test_image',
                        help="test image path", default="images/office1.jpg", type=str)
    ...
    bboxes, landmarks = create_mtcnn_net(image, args.mini_face, device, p_model_path='weights/pnet_Weights',
                                         r_model_path='weights/rnet_Weights', o_model_path='weights/onet_Weights')
    ...
    if landmarks is not None:
        for i in range(landmarks.shape[0]):
            ...
            for j in range(5):
                cv2.circle(image, (int(landmark[j, 0]), int(
                    landmark[j, 1])), 2, (0, 255, 255), 1)
